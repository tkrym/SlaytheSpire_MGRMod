...
public abstract class AbstractNote extends AbstractOrb
{
    ...
    public static AbstractNote GetRandomBasicNote()
    {
        ...
        if (!AbstractDungeon.player.hasPower(BrilliantPower.POWER_ID))
        {
            if (seed < 35) return new AttackNote();//35%
            else if (seed < 70) return new DefendNote();//35%
            else if (seed < 78) return new DebuffNote();//8%
            else if (seed < 95) return new DrawNote();//17%
        ...
        else
        {
            ...
            else if (seed < 83) return new DrawNote();//19%
            else if (seed < 93) return new DebuffNote();//10%
